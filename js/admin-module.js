...
function status(t,e){const x=$("pageStatus");if(!x)return;x.textContent=t||"";x.className="status"+(e?" error":"");}
function esc(v){return String(v==null?"":v).replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));}

async function rpc(name,params){
 const c=C();if(!c)throw Error("Supabase client unavailable.");
 const r=await c.schema("albukhr_security").rpc(name,params||{});
 if(r.error)throw r.error;return r.data;
}

function renderApprovalQueue(records){
 const host=$("workspaceText");if(!host)return;
 if(!records.length){
 ...
 host.querySelectorAll("[data-action]").forEach(btn=>{
  btn.addEventListener("click",async()=>{
   const card=btn.closest(".approval-item"),id=card&&card.dataset.projectId;if(!id)return;
...
async function loadApprovalQueue(){
 ...
 if(!result||result.authorized!==true)throw Error("Core Project approval authorization denied.");
 renderApprovalQueue(Array.isArray(result.records)?result.records:[]);
}

async function loadHistory(card,projectId){
 const box=card.querySelector(".approval-history");if(!box)return;
 box.hidden=false;box.textContent="Loading approval history...";
 try{
  ...
  ).join(""):"<span>No Core approval decisions recorded.</span>";
 }catch(e){console.error(e);box.textContent=e&&e.message?e.message:"Unable to load approval history.";}
}
...
async function approveProject(projectId,button){
 ...
 button.disabled=true;status("Submitting approval to the server...");
 try{
  const result=await rpc("approve_core_project",{p_project_id:projectId,p_reason:reason});
  if(!result||result.success!==true)throw Error("Server did not confirm the approval.");
  status("Core Project approved successfully. Project status is now APPROVED.");
  await loadApprovalQueue();
 }catch(e){
  console.error(e);status(e&&e.message?e.message:"Core Project approval failed.",true);button.disabled=false;
 }
}
...
async function init(){
 try{
  ...
  await A().init();
  const a=await A().requireAdmin({redirect:false});
  if(!a){location.replace("admin-login.html");return;}
  const m=await A().ensureMfa();
  if(a.mfa_required&&!m.verified){location.replace("admin-mfa.html");return;}

  const key=new URLSearchParams(location.search).get("module");
  const mod=MODULES[key];
  if(!mod){location.replace("admin-dashboard.html");return;}
  ...
  console.error(e);
  status(e&&e.message?e.message:"Admin authorization failed.",true);
 }
}
